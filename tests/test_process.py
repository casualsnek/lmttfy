"""Tests for :mod:`lmttfy.process` — :class:`MultiProcessedCall`."""
...
class TestInvokeInSP:
    ...
    def test_multiple_concurrent_calls(self):
        """Multiple concurrent subprocess calls all resolve correctly."""
        f = invoke_in_sp()(sleep_n_add)
    ...
    def test_wait_with_timeout(self):
        """wait(timeout=…) returns the correct value when the process completes within the timeout."""
        f = invoke_in_sp()(sleep_n_add)
        result = f(0.05, 42, 1)
        assert result.wait(timeout=5) == 43

    def test_wait_timeout_expires(self):
        """wait(timeout=…) returns None when the process hasn't completed yet — the value is
...
class TestMultiProcessedCallErrors:
    ...
    def test_burst_raises_captured_exception(self):
        """burst() raises the exception that occurred inside the subprocess function."""
    ...
    def test_on_error_callback(self):
        """The on_error callback fires when the subprocess function raises."""
        f = invoke_in_sp()(raise_value_error)
...
class TestMultiProcessedCallConcurrency:
    ...
    def test_max_concurrent_calls_exceeded(self):
        ...
        f = invoke_in_sp(max_concurrent_execs=2)(sleep_n_add)
    ...
    def test_max_concurrent_recovery(self):
        """After a subprocess completes, the counter frees up so a new call can be made."""
        f = invoke_in_sp(max_concurrent_execs=1)(sleep_n_add)
